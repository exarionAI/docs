...
### Unity Audio 설정 (image: unity-audio-settings-stereo-best-latency)
...
3. 소스 역할 GameObject를 만들고 `SoundTraceSource`를 붙입니다. Unity `AudioSource`는 자동으로 요구됩니다.
...
10. 리스너나 소스 위치를 움직여 소리가 물리적으로 변하는지 확인합니다.
11. `SoundTraceObject`의 `Update Mode`가 `Static`이면 Transform을 이동해도 geometry update 용도로 재빌드하지 않습니다.
12. 이동하거나 runtime shape update가 필요한 오브젝트는 `Refit`으로 설정합니다.
13. `Rebuild`는 형상이 실제로 크게 바뀌는 경우에만 사용하고, 매 frame 호출되는 구성은 피합니다.
...
| `primitivesPerLeaf` | 마지막 leaf node 안에 들어가는 triangle 개수입니다. 작게 잡을수록 detail은 좋아지지만 build/traversal cost가 달라집니다. |
| `Static` | 정적 geometry용입니다. runtime 이동/형상 변경을 반영하지 않는 구성에 사용합니다. |
| `Refit` | 기존 구조를 유지하면서 runtime transform 또는 shape update를 따라가야 하는 오브젝트에 사용합니다. |
| `Rebuild` | topology나 형상이 크게 바뀌어 BVH를 다시 만들어야 할 때만 사용합니다. |
...
| 설정 | 설명 |
|---|---|
| `Ray Resolution` | 발사하는 listener guide ray 해상도입니다. 범위는 `1-64`이며, 하나의 값이 native listener width와 height에 동일하게 적용됩니다. 값이 `16`이면 `16 x 16` guide ray를 사용합니다. |
...
| `Reverb Rays` | 소스의 잔향 ray 설정입니다. `Ray Resolution` 범위는 `1-64`이며 reverb ray width와 height에 동일하게 적용됩니다. `Reverb Ray Depth` 범위는 `1-16`입니다. listener ray와 별도 설정입니다. |
