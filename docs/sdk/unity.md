...
| 설정 | 설명 |
|---|---|
| `Ray Count Width`, `Ray Count Height` | 발사하는 listener guide ray 해상도입니다. 값이 커질수록 전체 ray 수와 비용이 증가합니다. |
| `Ray Depth` | ray 반사/전파 depth입니다. 최대 `16`입니다. 값이 클수록 잔향감과 복잡한 path 표현이 좋아지지만 연산량도 증가합니다. |
| `Per-path enable` | `Direct`, `Reflection`, `Diffraction`, `Reverb`, `Transmission` path를 타입별로 켜고 끕니다. |

복잡한 게임 씬의 초기값은 `16 x 16 x 4`를 권장합니다.
...
| `Reverb Rays` | 소스의 잔향 ray 해상도와 depth입니다. listener ray와 별도 설정입니다. |
