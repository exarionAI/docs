...
12. `SoundTraceObject`의 `Update Mode`가 `Static`이면 Transform을 이동해도 geometry update 용도로 재빌드하지 않습니다.
13. 이동이 필요한 오브젝트는 `Refit`으로 설정합니다.

![이동 가능한 오브젝트 설정](/img/unity/Image09_Movable.png)

14. `Rebuild`는 형상이 실제로 크게 바뀌는 경우에만 사용하고, 매 frame 호출되는 구성은 피합니다.
