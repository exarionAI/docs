...
두 모드 모두 브라우저가 `crossOriginIsolated === true`여야 합니다. HTML 응답에 다음
헤더를 설정하세요.
...
이 경우 BVH는 refit 가능한 `LBVH` 계열로 빌드하세요. 프레임마다
...
| 필드 | 기본값 | 권장 범위 | 설명 |
|---|---:|---:|---|
| `bvhType` | `BvhType.Default` | 아래 enum | `Default(-1)`는 현재 native `ExaMeshBuildOption.bvhType` 사용 |
| `bvhMaxDepth` | `0` | `0` 또는 `1..32` | `0` 또는 미지정은 native default. 양수는 해당 mesh의 트리 최대 깊이 |
| `primPerLeaf` | `0` | `0` 또는 `1..32` | `0` 또는 미지정은 native default. 양수는 leaf 하나당 primitive 수 |

BVH 선택 기준:

| 타입 | 값 | 쓰임 |
|---|---:|---|
| `Default` | `-1` | `SoundMesh.setData()`에서 현재 native default mesh build option 사용 |
| `HKDtree` | `0` | **정적 사운드 콜라이더**. 벽, 방, 바닥처럼 topology와 vertex가 고정된 메시. 현재 엔진에서 `SBVH`의 대용으로 존재 |
| `LBVH` | `1` | 동적·스키닝 콜라이더 기본 선택. vertex가 프레임마다 바뀌고 scene tick refit 경로가 필요한 메시 |
...
`defaultMeshBuild.bvhType`은 native process-wide default라서 실제 builder enum을
넣어야 합니다. `BvhType.Default`는 per-mesh `setData()`에서 “현재 native default를
따른다”는 sentinel로만 쓰세요.
...
`LBVH`입니다. 더 넓은 SIMD/N-way builder가 필요하면 collider 옵션의 `bvhType`을
명시합니다. 스키닝 애니메이션은 topology를 유지하고 매 프레임
...
| `render(sourceID, input, output, channelCount)` | 저수준 수동 렌더. 일반 Web Audio 통합은 `createWorkletNode()` 사용 |
...
| `inputSampleCount` | `128` | `createWorkletNode()` 경로에서 엔진이 한 번에 처리할 frame 수 |
...
| `setJobTimingOption({ enabled, frameCapacity })` | native propagation job timing ring buffer 설정 |
| `getJobTimingFrames(sceneID, maxFrames?)` | 최근 propagation frame/job timing snapshot 조회 |
...
Vite dev server는 기본적으로 COOP/COEP 헤더를 설정하므로 ST/MT 모드를 모두 확인할
수 있습니다. 문서 iframe은 시작 기본값을 명확히 하기 위해 `?thread=st`로 고정해
둡니다.
...
이 경로는 `LBVH` 계열 builder와 `updateVertices + UpdateType.Refit` 조합을 보여주기
위한 것입니다. 데모에서 BVH 타입을 `HKDtree`로 바꾸면 Flair는 bind pose 기반의
정적 검사용으로 취급하세요.
...
| 증상 | 확인할 것 |
|---|---|
| WASM 로드 실패 | HTML 응답에 COOP/COEP가 있는지, `crossOriginIsolated`가 `true`인지 확인 |
...
| 애니메이션 collider가 튐 | `LBVH` 계열, `updateVertices()`, `object.setUpdateType(UpdateType.Refit)`, `scene.tick()` 흐름인지 확인 |
...
이 증상은 보통 STCoreV2 core 자체보다 초기화 순서, audio option, 또는 앱
...
- 저수준 `listener.render()`를 직접 호출할 때 `channelCount`는 `2`, 입력 버퍼 길이는 `frames * 2`인 interleaved sample 수여야 합니다. frame 수만 넘기거나 mono buffer를 그대로 넘기면 엔진의 mono mix/render buffer 계약과 어긋납니다.
