...
3. **listener audio option을 실제 context와 맞춥니다.** `sampleRate`는 `ctx.sampleRate`, `outputChannels`는 현재 실시간 HRTF 경로 기준 `2`로 설정합니다. `inputSampleCount`는 MT worklet이면 `128`, ST fallback이면 앱의 render block size와 동일하게 맞춥니다.
