...
3. **listener audio option 要匹配真实 context。** `sampleRate` 使用 `ctx.sampleRate`，当前 real-time HRTF path 的 `outputChannels` 使用 `2`。MT worklet 的 `inputSampleCount` 使用 `128`；ST fallback 则要与 app 的 render block size 一致。
