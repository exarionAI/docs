...
Unity SDK の 3 つのサンプルと同じ構成を、Unreal のレベルとブループリントで提供します。
Content Browser で `Settings > Show Plugin Content` を有効にし、
`SoundTracing Content > Samples > Maps` のレベルを開いて `Play` を押します。
各レベルには専用の GameMode とカメラが設定されており、実行すると音声が自動再生されます。

3 つのデモに共通する UI は次のとおりです。

| ボタン | キーボード | 動作 |
|---|---|---|
| `Play` | `P` | 音楽または会話を最初から再生します。 |
| `Pause / Resume` | `Space` | 一時停止、または続きから再生します。 |
| `Stop` | `X` | 再生を停止します。 |
| `Reset` | `R` | デモを再開します。02 では位置と音響マテリアルも初期化します。 |
| `Show / hide UI` | — | 左側の操作パネルを非表示・表示にします。 |

レベルは `/SoundTracing/Samples/Maps`、動作と UI のブループリントは
`/SoundTracing/Samples/Blueprints` にあります。音源ごとの設定は
`/SoundTracing/Samples/Audio/Settings` で編集します。

### ST_SampleScene01

![ST_SampleScene01 — 部屋の中の単一音源とリスナー](/img/unreal/ST_Sample01.png)

10 m の部屋で、1 つの音源と固定リスナーを使って基本的な空間音響を確認します。
音楽を聴きながら、部屋のジオメトリ、音響マテリアル、可視化されたパスと音の関係を確認できます。

`SoundTrace / Unreal dry` ボタンで SoundTrace 出力と Unreal の通常出力を切り替えます。
両方の再生位置が同期しているため、音楽を続けて聴きながら空間の広がりや残響を比較できます。

動作のブループリントは `BP_Sample01`、UI は `WBP_Sample01` です。

### ST_SampleScene02

![ST_SampleScene02 — 楽器音源とリスナーを動かすトップビューデモ](/img/unreal/ST_Sample02.png)

トップビューでギター・ベース・ドラム・シンセの左右 8 音源と、ヘッドホン形のリスナーを移動します。
Quartz で音楽を同期再生し、配置や音響マテリアルを変えながら方向感と空間の応答を比較できます。

| 操作 | 動作 |
|---|---|
| アイコンをドラッグ | マウスの左ボタン、または最初のタッチで音源・リスナーを移動します。高さと最初につかんだ位置のオフセットを維持します。 |
| `Mirror: ON / OFF` | 音源をドラッグすると、反対側のペアも左右対称に移動します。有効にした瞬間に位置を揃えることはありません。 |
| `SoundTrace / Unreal dry` | 再生位置を保ちながら SoundTrace 出力と Unreal の通常出力を切り替えます。 |
| `Acoustic material (floor + dome)` | 床とドームの音響マテリアルを一緒に変更します。初期プリセットは `Glass` で、見た目のマテリアルとは別の設定です。 |
| `Reset` / `R` | 音源・リスナーの位置、`Mirror: ON`、音響マテリアル `Glass` を復元して再生を再開します。 |
| `Record / save` / `F9` | 出力の録音を開始します。録音中にもう一度押すと WAV に保存して録音を終了します。 |
| `Save WAV` / `F10` | 録音中の内容を WAV に保存して録音を終了します。 |

録音ファイルはプロジェクトの `Saved/BouncedWavFiles` に一意の名前で保存されます。
Unreal では WAV を保存すると現在の録音が終了します。Unity の録音中のスナップショット書き出しとは操作が異なります。
UI 上や画面端から 24 px 以内で始めたドラッグは処理されないため、アイコンの内側からドラッグを始めてください。

動作のブループリントは `BP_Sample02`、UI は `WBP_Sample02` です。

### ST_SampleScene03

![ST_SampleScene03 — 壁と仕切りのある空間の男女音声デモ](/img/unreal/ST_Sample03.png)

壁と仕切りのある空間で、男性・女性の 2 つの音源が 8 つの音声クリップを交互に繰り返し再生します。
カメラと一緒にリスナーを動かし、壁による遮蔽、反射、HRTF の方向感、空間の残響の変化を確認します。
...
| `Q / E` | 下降・上昇します。 |
| マウス移動 | 視点を回転します。 |
| `Esc` | 視点操作を解除して UI を操作します。エディター PIE では `Shift+F1` でカーソルを解放します。 |
| マウス右クリック | 視点操作を再開します。 |
| `Reset` / `R` | 現在のカメラ位置で会話を最初から再生します。 |

エディター PIE では `Esc` がプレイ終了に使われます。
動作のブループリントは `BP_Sample03`、UI は `WBP_Sample03` です。
