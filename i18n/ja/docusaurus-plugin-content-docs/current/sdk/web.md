...
静的colliderのdefault BVHは`HKDtree`、`dynamic: true`またはskinning colliderのdefault BVHは`LBVH`です。skinning animationではtopologyを維持し、毎フレーム`collider.refitVertices(vertices)`を呼びます。
...
3. **listener audio optionを実際のcontextと合わせます。** `sampleRate`は`ctx.sampleRate`、現在のreal-time HRTF pathでは`outputChannels`は`2`です。MT workletの`inputSampleCount`は`128`、ST fallbackではappのrender block sizeと一致させます。
