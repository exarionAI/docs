...
3. **listener audio option 要匹配真實 context。** `sampleRate` 使用 `ctx.sampleRate`，目前 real-time HRTF path 的 `outputChannels` 使用 `2`。MT worklet 的 `inputSampleCount` 使用 `128`；ST fallback 則要與 app 的 render block size 一致。
