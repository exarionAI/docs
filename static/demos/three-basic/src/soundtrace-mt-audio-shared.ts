...
export function createMtAudioSharedBuffers(
  channels: number,
  options: {
    readonly blockFrames?: number;
    readonly capacityFrames?: number;
...
export function createMtAudioRingView(ring: MtAudioRing): MtAudioRingView {
...
export function getReadableFrameCount(view: MtAudioRingView): number {
...
export function getWritableFrameCount(view: MtAudioRingView): number {
  return view.capacityFrames - getReadableFrameCount(view);
}

export function writeInterleavedFrames(
  view: MtAudioRingView,
  source: Float32Array,
  frameCount: number,
): boolean {
  ...
  if (source.length < frameCount * view.channels) {
    throw new Error(
      `[soundtrace.js] source length ${source.length} is too small for ${frameCount} frames x ${view.channels} channels`,
    );
  }
...
export function readInterleavedFrames(
  view: MtAudioRingView,
  ...
  if (destination.length < frameCount * view.channels) {
    throw new Error(
      `[soundtrace.js] destination length ${destination.length} is too small for ${frameCount} frames x ${view.channels} channels`,
    );
  }
...
function validateAudioDimensions(
  ...
  if (!Number.isInteger(capacityFrames) || capacityFrames < blockFrames) {
    throw new Error(
      `[soundtrace.js] capacityFrames must be an integer >= blockFrames (got ${capacityFrames}, blockFrames=${blockFrames})`,
    );
  }
}

function createMtAudioRing(
  channels: number,
  capacityFrames: number,
): MtAudioRing {
  return {
...
function copyFramesIntoRing(
  ...
  for (let frameIndex = 0; frameIndex < frameCount; frameIndex += 1) {
    ...
    for (let channelIndex = 0; channelIndex < frameStride; channelIndex += 1) {
      ...
      if (sample === undefined) {
        throw new Error(
          `[soundtrace.js] missing input sample for frame ${frameIndex} channel ${channelIndex}`,
        );
      }
