...
export async function executeCreateMesh(
  ...
  try {
    ...
    return commandFailure(
      ...
      context,
      `[soundtrace.js] createMesh failed for handle ${command.command.handle}`,
    );
  }
}

export async function executeDeleteMesh(
  command: CommandRequest,
  context: ControlWorkerCommandContext,
): Promise<CommandExecutionResult> {
  ...
  try {
    ...
    return commandFailure(
      ...
      error instanceof Error ? error : context.normalizeCaughtUnknown(error),
      context,
      `[soundtrace.js] deleteMesh failed for handle ${command.command.handle}`,
    );
  }
}

export async function executeSetMeshMaterial(
  command: CommandRequest,
  context: ControlWorkerCommandContext,
): Promise<CommandExecutionResult> {
  ...
  try {
    ...
    return commandFailure(
      ...
      error instanceof Error ? error : context.normalizeCaughtUnknown(error),
      context,
      `[soundtrace.js] setMeshMaterial failed for handle ${command.command.handle}`,
    );
  }
}

export async function executeSetMeshMaterialRange(
  command: CommandRequest,
  context: ControlWorkerCommandContext,
): Promise<CommandExecutionResult> {
  ...
  try {
    ...
    return commandFailure(
      ...
      error instanceof Error ? error : context.normalizeCaughtUnknown(error),
      context,
      `[soundtrace.js] setMeshMaterialRange failed for handle ${command.command.handle}`,
    );
  }
}

export async function executeSetMeshUpdateType(
  command: CommandRequest,
  context: ControlWorkerCommandContext,
): Promise<CommandExecutionResult> {
  ...
  try {
    ...
    return commandFailure(
      ...
      error instanceof Error ? error : context.normalizeCaughtUnknown(error),
      context,
      `[soundtrace.js] setMeshUpdateType failed for handle ${command.command.handle}`,
    );
  }
}
