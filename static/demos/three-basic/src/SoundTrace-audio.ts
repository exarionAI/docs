...
export async function playSource(self: SoundTrace, source: Source, channels: number): Promise<AudioWorkletNode> {
...
export async function createWorkletNode(self: SoundTrace, listener: SoundListener, source: SoundSource, channels = 2): Promise<AudioWorkletNode> {
  await self._ensureWorkletReady();
  const handle = self.bindings().exa_audio_worklet_create_node(
    self._ctxHandle, listener.id, source.id, channels);
  if (!handle) {
    throw new Error(
      '[soundtrace.js] exa_audio_worklet_create_node returned 0 — init incomplete or alloc failed');
  }
...
export async function createMtAudioWorkletNode(self: SoundTrace, sourceHandle: number, channels: number): Promise<AudioWorkletNode> {
  ...
  const node = new AudioWorkletNode(
    self.ctx,
    MT_AUDIO_WORKLET_PROCESSOR_NAME,
    {
      numberOfInputs: 1,
      numberOfOutputs: 1,
...
export function pumpMtControlLoop(self: SoundTrace): void {
...
export async function createMixerWorkletNode(self: SoundTrace, listener: SoundListener, sources: readonly SoundSource[], channels = 2): Promise<AudioWorkletNode> {
  if (sources.length === 0) {
  ...
  const handle = heap.withScope((scope) => {
    ...
    return self.bindings().exa_audio_worklet_create_mixer_node(
      self._ctxHandle, listener.id, idsPtr, ids.length, channels);
  });
  if (!handle) {
    throw new Error(
      '[soundtrace.js] exa_audio_worklet_create_mixer_node returned 0 — init incomplete or alloc failed');
  }
...
export function audioWorkletNodeFromHandle(self: SoundTrace, handle: number): AudioWorkletNode {
  const mod = self.requireModule();
  if (!mod.emscriptenGetAudioObject) {
    throw new Error(
      '[soundtrace.js] emscriptenGetAudioObject not exported (build missing -sAUDIO_WORKLET=1)');
  }
  const node = mod.emscriptenGetAudioObject(handle);
  if (!(node instanceof AudioWorkletNode)) {
    throw new Error(
      `[soundtrace.js] unexpected object type for worklet node handle ${handle}`);
  }
  return node;
}

export function ensureWorkletReady(self: SoundTrace): Promise<void> {
  if (self._workletInitPromise) return self._workletInitPromise;
  self._workletInitPromise = self._bootstrapWorklet();
  return self._workletInitPromise;
}

export function bootstrapWorklet(self: SoundTrace): Promise<void> {
  const mod = self.requireModule();
  if (!mod.emscriptenRegisterAudioObject) {
    return Promise.reject(new Error(
      '[soundtrace.js] emscriptenRegisterAudioObject not exported (build missing -sAUDIO_WORKLET=1)'));
  }
  if (!mod.addFunction || !mod.removeFunction) {
    return Promise.reject(new Error(
      '[soundtrace.js] addFunction/removeFunction not exported (build missing -sALLOW_TABLE_GROWTH=1)'));
  }
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    const cleanup = () => {
      ...
      if (cbPtr !== 0) removeFunction(cbPtr);
    };
    ...
    timer = setTimeout(() => {
      cleanup();
      reject(new Error(
        '[soundtrace.js] audio worklet init timeout (5s). Verify that ' +
...
export function requireModule(self: SoundTrace): ExaSoundModule {
  self.assertLocalControlAvailable('audio worklet module access');
  if (!self.module) throw new Error('[soundtrace.js] not loaded — call load() first');
  return self.module;
}

export function ensureMtAudioWorkletReady(self: SoundTrace): Promise<void> {
  if (!self.ctx.audioWorklet?.addModule) {
    return Promise.reject(
      new Error('[soundtrace.js] AudioWorklet is not available on this AudioContext'),
    );
  }
...
export async function loadMtAudioWorkletModule(self: SoundTrace): Promise<void> {
