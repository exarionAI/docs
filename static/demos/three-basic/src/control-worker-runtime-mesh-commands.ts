...
export function createRuntimeMeshCommandAdapters(
  context: RuntimeCommandAdapterContext,
): Pick<
  RuntimeCommandAdapterSet,
  'createMesh' | 'deleteMesh' | 'setMeshMaterial' | 'setMeshMaterialRange' | 'setMeshUpdateType'
> {
  const { bindings, getSceneId, meshIdsByObjectId, heap, requireNativeSuccess } = context;

  const applyMeshPose = (objectId: number, pose: unknown): void => {
    if (!isRecord(pose)) {
      ...
      try {
        ...
        heap.withScope((scope) => {
          ...
          requireNativeSuccess(
            bindings.exaMeshSetData(meshId, vertsPtr, numVerts, trisPtr, triangles.length, 0, 0, command.initial.bvhType),
            `failed to set mesh data for mesh ${meshId}`,
          );
        });
...
function readTrianglesPayload(value: readonly Record<string, unknown>[]): Triangle[] {
