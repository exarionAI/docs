import type { CommandCreateSource, CommandSetSourceParam, SourcePathType } from './control-protocol.js';
...
const SOURCE_PATH_TYPES: ReadonlyArray<readonly [SourcePathType, number]> = [
  ['direct', 0],
  ['reflection', 1],
  ['diffraction', 2],
  ['reverberation', 3],
...
export function createRuntimeSourceCommandAdapters(
  context: RuntimeCommandAdapterContext,
): Pick<RuntimeCommandAdapterSet, 'createSource' | 'deleteSource' | 'setSourceParam'> {
  const { bindings, getSceneId, heap, requireNativeSuccess } = context;

  const applySourceDistanceAttenuation = (sourceId: number, value: unknown): void => {
    if (!isRecord(value)) {
    ...
    heap.withScope((scope) => {
      ...
      requireNativeSuccess(
        bindings.exaSoundSourceSetDistanceAttenuations(
          sourceId,
          directPtr,
          reflectionPtr,
          diffractionPtr,
        ...
        `failed to set source distance attenuation for source ${sourceId}`,
      );
    });
  };

  const applySourcePathOptions = (sourceId: number, value: unknown): void => {
    if (!isRecord(value)) {
    ...
    for (const [key, pathType] of SOURCE_PATH_TYPES) {
      ...
      if (enabled !== undefined) {
        requireNativeSuccess(
          bindings.exaSoundSourceSetPathEnable(sourceId, pathType, requireBoolean(enabled, `source path "${key}"`)),
          `failed to set source path ${key} for source ${sourceId}`,
        );
      }
    }
  };

  const applySourceInitialState = (command: CommandCreateSource, sourceId: number): void => {
    requireNativeSuccess(
      bindings.exaSoundSourceSetPosition(sourceId, ...command.initial.position),
      `failed to set source position for source ${sourceId}`,
    );
    requireNativeSuccess(
      bindings.exaSoundSourceSetDirection(sourceId, ...command.initial.direction),
      `failed to set source direction for source ${sourceId}`,
    );
    requireNativeSuccess(
      bindings.exaSoundSourceSetVelocity(sourceId, ...command.initial.velocity),
      `failed to set source velocity for source ${sourceId}`,
    );
    requireNativeSuccess(
      bindings.exaSoundSourceSetIntensity(sourceId, command.initial.intensity),
      `failed to set source intensity for source ${sourceId}`,
    );
    requireNativeSuccess(
      bindings.exaSoundSourceSetRayCount(sourceId, command.initial.rayCount[0], command.initial.rayCount[1]),
      `failed to set source ray count for source ${sourceId}`,
    );
    requireNativeSuccess(
      bindings.exaSoundSourceSetDepth(sourceId, command.initial.depth),
      `failed to set source depth for source ${sourceId}`,
    );
  ...
  return {
    async createSource(command) {
      ...
      try {
        ...
        requireNativeSuccess(
          bindings.exaSceneAddSource(getSceneId(), sourceId),
          `failed to add source ${sourceId} to scene ${getSceneId()}`,
        );
        return sourceId;
      } catch (error) {
    ...
    async deleteSource(command) {
      requireNativeSuccess(
        bindings.exaSceneRemoveSource(getSceneId(), command.engineId),
        `failed to remove source ${command.engineId} from scene ${getSceneId()}`,
      );
      bindings.exaDeleteSoundSource(command.engineId);
      return { deleted: true };
    },

    async setSourceParam(command: CommandSetSourceParam & { readonly engineId: number }) {
...
function applySourceParamPatch(
  ...
  applySourceDistanceAttenuation: (sourceId: number, value: unknown) => void,
  applySourcePathOptions: (sourceId: number, value: unknown) => void,
): void {
...
function toVec3(value: readonly [number, number, number]): { readonly x: number; readonly y: number; readonly z: number } {
