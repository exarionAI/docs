...
// The Emscripten MODULARIZE glue is a classic script (no ESM exports), and
// pthread workers reload the same file as a classic script too. That means
// ESM `import()` returns an empty namespace. Inject it via a <script> tag
// instead: `var ExaSoundModule` lands on globalThis, and the browser fills in
// `document.currentScript.src` so worker URL resolution works automatically.
...
function parseMemoryMaxMb(): number | null {
  if (typeof location !== 'undefined') {
    const requested = new URLSearchParams(location.search).get('wasmMemoryMaxMb');
    if (requested) {
      const value = Number(requested);
      if (Number.isFinite(value) && value >= 64 && value <= 2048) return value;
    }
  }
  return isIosWebKit() ? DEFAULT_IOS_SHARED_MEMORY_MAX_MB : null;
}

function createSharedWasmMemory(maxMb: number): WebAssembly.Memory {
...
function shouldDropNativeLog(message: string): boolean {
  return message.includes('[Exarion][Info] SetupPlaneUsage:');
}

function printNativeError(...args: unknown[]): void {
  const message = args.map((arg) => String(arg)).join(' ');
  if (shouldDropNativeLog(message)) return;
  console.error(...args);
}
...
export async function loadCore(
  ...
  const moduleOptions: Partial<ExaSoundModule> = {
    locateFile: (path: string) => `${baseUrl}/${thread}/${path}`,
