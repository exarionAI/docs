...
export async function executeCreateSource(command, context) {
    ...
    try {
        ...
        return commandFailure(command.id, error instanceof Error ? error : context.normalizeCaughtUnknown(error), context, `[soundtrace.js] createSource failed for handle ${command.command.handle}`);
    }
}
export async function executeDeleteSource(command, context) {
    ...
    try {
        ...
        return commandFailure(command.id, error instanceof Error ? error : context.normalizeCaughtUnknown(error), context, `[soundtrace.js] deleteSource failed for handle ${command.command.handle}`);
    }
}
export async function executeSetSourceParam(command, context) {
    ...
    try {
        ...
        return commandFailure(command.id, error instanceof Error ? error : context.normalizeCaughtUnknown(error), context, `[soundtrace.js] setSourceParam failed for handle ${command.command.handle}`);
    }
}
export async function executeStartAudioSource(command, context) {
    ...
    try {
        ...
        return commandFailure(command.id, error instanceof Error ? error : context.normalizeCaughtUnknown(error), context, `[soundtrace.js] startAudioSource failed for handle ${command.command.handle}`);
    }
}
export async function executeStopAudioSource(command, context) {
    ...
    try {
        ...
        return commandFailure(command.id, error instanceof Error ? error : context.normalizeCaughtUnknown(error), context, `[soundtrace.js] stopAudioSource failed for session ${command.command.sessionId}`);
    }
}
export async function executeSetRenderOption(command, context) {
    ...
    try {
        ...
        return commandFailure(command.id, error instanceof Error ? error : context.normalizeCaughtUnknown(error), context, `[soundtrace.js] setRenderOption failed for source ${command.command.source}`);
    }
}
function hasSourceTransformPatch(patch) {
    return Object.keys(patch).some((key) => key === 'position'
        || key === 'direction'
        || key === 'velocity'
        || key === 'intensity');
}
...
//# sourceMappingURL=control-worker-command-source.js.map