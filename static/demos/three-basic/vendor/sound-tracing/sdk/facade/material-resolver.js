...
/**
 * Build the worker material payload from the loaded material asset, preserving
 * array order so the worker registers each material at the same index the ST
 * facade resolves names to. Arrays are copied into plain number[] so the
 * payload is structured-clone-safe across the worker boundary.
 */
export function buildWorkerMaterialPayload(asset) {
    return asset._soundMaterials.map((entry) => ({
        reflection: Array.from(entry.reflection, Number),
        absorption: Array.from(entry.absorption, Number),
        transmission: Array.from(entry.transmission, Number),
        scattering: entry.scattering,
        index: entry.materialType,
    }));
}
