import { RAY_SIZE, RAYHIT_SIZE, TRIANGLE_SIZE, VEC3_SIZE, isError, isOk, readRayHit, requireOk, validateMeshGeometry, writeRay, writeTriangles, } from '../native/index.js';
// Mirrors the v0.7 ExaBvhType values (exaSoundC.h). The experimental CWBVH
// slot was never part of the public ExaBvhType range and was dropped with the
// clean break.
...
export class SoundMesh {
    ...
    constructor(b, h) {
        ...
        this.id = h.withScope(s => {
            const p = s.i32();
            requireOk(b.exaMeshCreate(p), 'exaMeshCreate', b.exaGetLastError);
            return h.readI32(p);
        });
    }
    /**
    ...
    setData(vertices, triangles, opts = {}) {
        ...
        return this.h.withScope(s => {
            ...
            const ok = isOk(this.b.exaMeshSetData(this.id, vPtr, numVerts, tPtr, triangles.length, bvhMaxDepth, primPerLeaf, bvhType));
    ...
    updateVertices(vertices) {
        ...
        return isOk(this.b.exaMeshUpdateVertices(this.id, this.updateVertsPtr, numVerts));
    }
    updateVerticesAndRefit(vertices) {
        ...
        this.h.writeF32Array(this.updateVertsPtr, vertices);
        return isOk(this.b.exaMeshUpdateVertices(this.id, this.updateVertsPtr, numVerts))
            && isOk(this.b.exaMeshRefit(this.id));
    }
    refit() { return isOk(this.b.exaMeshRefit(this.id)); }
    setMaterial(materialIndex) {
        return isOk(this.b.exaMeshSetMaterial(this.id, materialIndex));
    }
    setMaterialRange(triStart, triCount, materialIndex) {
        return isOk(this.b.exaMeshSetMaterialRange(this.id, triStart, triCount, materialIndex));
    }
    /**
     ...
     *
     * Backed by v0.7 `exaMeshGetTriangleVertices` (renamed from
     * exaGetSoundMeshVrtsTrisAll). The engine-owned pointer is invalidated by
     * `setData()` / `refit()` — this method copies the data out immediately.
     *
    ...
    getBVHWireframe() {
        return this.h.withScope(s => {
            ...
            const result = this.b.exaMeshGetTriangleVertices(this.id, ptrPtr, countPtr);
            if (isError(result))
                return null;
    ...
    /** Intersect a ray against the scene the mesh belongs to (v0.7
     *  exaSceneRaycast — a miss is a SUCCESS with hit == 0). Returns null on a
     *  miss or when the engine could not service the call. */
    intersect(sceneID, ray) {
        return this.h.withScope(s => {
            const rPtr = s.block(RAY_SIZE);
            writeRay(this.h, rPtr, ray);
            const hitPtr = s.block(RAYHIT_SIZE);
            const result = this.b.exaSceneRaycast(sceneID, rPtr, hitPtr);
            if (isError(result))
                return null;
            const hit = readRayHit(this.h, hitPtr);
