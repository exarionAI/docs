...
import { HrtfMode, isDelayInterpolation, isDiffuseQuality, isHrtfQuality, isLateReverbMode, isSupportedHrtfMode } from './SoundListener-options.js';
...
export class SoundListener extends SoundListenerBase {
    ...
    render(sourceID, input, output, channelCount) {
        ...
        const ok = this.b.exaRenderSound(this.id, sourceID, this.inPtr, input.length, frameCount, channelCount, this.outPtr, output.length);
        if (ok)
    ...
    setMaxDelay(sourceID, v) {
        return this.b.exaSetMaxDelay(this.id, sourceID, v);
    }
    setPathFadeTime(sourceID, v) {
    ...
    setHrtfMode(mode, data) {
        if (mode === HrtfMode.Parametric)
            return this.loadParametricHrtf(data);
        if (mode === HrtfMode.Convolution)
            return this.loadConvolutionHrtf(data);
        throw new Error('[soundtrace.js] Unsupported HRTF mode; use parametric or convolution');
    }
    loadParametricHrtf(data) {
        const loaded = this.loadHrtfTable(data, (ptr, length) => this.b.exaListenerLoadParametricHrtf(this.id, ptr, length));
        return loaded ? Boolean(this.b.exaListenerSetHrtfMode(this.id, HrtfMode.Parametric)) : false;
    }
    loadConvolutionHrtf(data) {
        const loaded = this.loadHrtfTable(data, (ptr, length) => this.b.exaListenerLoadConvolutionHrtf(this.id, ptr, length));
        return loaded ? Boolean(this.b.exaListenerSetHrtfMode(this.id, HrtfMode.Convolution)) : false;
    }
    loadHrtfTable(data, load) {
        if (data.byteLength === 0)
            return Boolean(load(0, 0));
        return this.h.withScope(s => {
