...
import { isRecord, readIntegerPair, requireBoolean, requireInteger, requireNumber, } from './control-worker-runtime-command-shared.js';
...
function applyListenerPathOptions(listenerId, value, bindings, requireNativeSuccess) {
    ...
    for (const [key, pathType] of LISTENER_PATH_TYPES) {
        ...
        if (enabled !== undefined) {
            requireNativeSuccess(bindings.exaListenerSetPathEnable(listenerId, pathType, requireBoolean(enabled, `listener path "${key}"`)), `failed to set listener path ${key} for listener ${listenerId}`);
        }
    }
}
